...
## Creates a chess piece class that shows what team a piece is on, what type of piece it is and whether or not it can be killed 
# by another selected piece.
class Piece:
...
## This takes in a piece object and its index then runs then checks where that piece can move 
# using separately defined functions for each type of piece.
...
## Basically, check black and white pawns separately and checks the square above them. 
# If it's free, that space gets an "x" and if it is occupied by a piece of the opposite team then that piece becomes killable.
...
# it takes a tuple argument which is the dimensions of the window so in this case 800 x 800px
...
#THey were put into a 2d array which is identical to the dimesions of the chessboard
